...
观察到 Base 链上其他两家发射平台的代币，即便官方提供了流动性深、手续费合理的 v4 Hook 池，Uniswap 前端默认仍然把用户导向第三方创建的无 Hook 池。这些无 Hook 池的流动性往往很浅、手续费很高，但官方界面和聚合器优先走的仍是这条路径。即使用户在前端手动切换到 “Only v4” 或 “Only Hooks”，大部分代币依然无法通过 Hook 池成交；如果使用 Uniswap X 路由则完全不会触达 Hook 池。

本文件的目的，是梳理 Uniswap 的两套路由机制、解释为何 Hook 池在默认情况下常被忽略，以及总结在项目落地时需要满足的条件。
...
3. **强制选择也不稳定**：即便在界面手动切换为 “Only v4” 或 “Only Hook”，部分代币依旧无法走到 Hook 池，说明即使用户强制选路，模拟阶段若仍因 `hookData`、风险标记或成本评估失败，路由器仍会放弃 Hook。
...
3. **对接外部聚合器与生态前端**  
   - 主动联系 1inch、OKX Web3、Matcha 等聚合器团队，提交 Hook 合约、审计报告、仿真脚本等材料，申请进入其协议白名单。  
